...
static esp_err_t event_handler(void *ctx, system_event_t *event)
{
    ...
    case SYSTEM_EVENT_STA_GOT_IP:
        auto got_ip = ip4addr_ntoa(&event->event_info.got_ip.ip_info.ip);
        dbg(got_ip);
...
struct http_server {
    ...
    static void connect (char const *ssid, char const *password)
    {
        tcpip_adapter_init();

        dbg(esp_event_loop_init(event_handler, &instance.server));
        ...
        dbg(esp_wifi_set_config(ESP_IF_WIFI_STA, &wifi_config));
